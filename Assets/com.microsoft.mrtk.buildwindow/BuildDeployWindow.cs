...
namespace Microsoft.MixedReality.Toolkit.Build.Editor
{
    ...
    public class BuildDeployWindow : EditorWindow
    {
        ...
        public delegate void ErgonomicsEvaluationDelegate();
        public static event ErgonomicsEvaluationDelegate OnErgonomicEvaluation;
        public static event ErgonomicsEvaluationDelegate OnErgoEvaluation;

        public delegate void ProgressBarDelegate();
        public static event ProgressBarDelegate e_progressBar;
        ...
        public static bool isAppxBuilding = false;
        public static async void BuildAll(bool install = true)
        {
            Debug.Log($"Build started : {DateTime.Now}");
            Debug.Assert(!isBuilding);
            isBuilding = true;
            ...
            //e_progressBar?.Invoke();

            bool buildSuccessfull = await UwpPlayerBuildTools.BuildPlayer(BuildDeployPreferences.BuildDirectory, false, appxCancellationTokenSource.Token);
            // First build SLN
            if (!buildSuccessfull)
            {
                Debug.LogError("Build Failed!");
                BuildFailure();
                OnErgonomicEvaluation?.Invoke();
                return;
            }
        ...
        public static async Task RefreshItems()
        {
            await Task.Delay(3000);
            OnErgoEvaluation?.Invoke();
        }
        ...
        public static void BuildSuccess()
        {
            ...
            e_progressBar?.Invoke();
            OpenBuild?.Invoke();
            Debug.Log($"Build Completed : {DateTime.Now}");
        }

        public static Action OpenBuild;
