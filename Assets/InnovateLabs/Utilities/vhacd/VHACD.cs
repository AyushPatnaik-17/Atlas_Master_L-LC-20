...
namespace MeshProcess
{
    public class VHACD : MonoBehaviour
    {
        ...
        [ContextMenu("Create Convex Meshes")]
        public void GenerateMeshCollider()
        {
            ...
            foreach(var mesh in meshFilters)
            {
                ...
                var isCollider = mesh.GetComponent<Collider>();
                if (isCollider)
                {
                    if(isCollider.GetType() != typeof(MeshCollider))
                    {
                        isCollider.isTrigger = true;

                        var rb = isCollider.gameObject.AddComponent<Rigidbody>();
                        rb.isKinematic = true;
                        rb.useGravity = false;

                        var cl = isCollider.gameObject.AddComponent<PartCollisionListener>();
                        continue;
                    }
                    else
                    {
                        DestroyImmediate(isCollider);
                    }
                }

                var convexMesh = GenerateConvexMeshes(mesh.sharedMesh);
                for(int i  = 0; i < convexMesh.Count; i++)
                {
                    var convexElement = new GameObject();
                    convexElement.name = $"{mesh.gameObject.name} {count}--{i}";
                    convexElement.transform.parent = mesh.transform;
                    convexElement.transform.localPosition = Vector3.zero;
                    convexElement.transform.localRotation = Quaternion.identity;
                    convexElement.transform.localScale = Vector3.one;

                    var collider = convexElement.AddComponent<MeshCollider>();
                    collider.sharedMesh = convexMesh[i];
                    collider.convex = true;
                    collider.isTrigger = true;

                    var rb = convexElement.AddComponent<Rigidbody>();
                    rb.isKinematic = true;
                    rb.useGravity = false;

                    var cl = convexElement.AddComponent<PartCollisionListener>();
                }
            }
        }
        [ContextMenu("Destroy Convex Meshes")]
